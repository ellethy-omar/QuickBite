...
export default function DriverDetailsScreen() {
  ...
  const handleUserBan = async () => {
    try {
      await banUserAccess(driverData._id, driverData.banned);
      showNotification(`User ${driverData.banned ? "unbanned" : "banned"} successfully!`, "success");
    } catch {
      showNotification(`An error occurred while ${driverData.banned ? "unbanning" : "banning"} user, please try again`, "error");
    } finally {
  ...
  useEffect(() => {
    ...
    const handleMessageSend = async () => {
      ...
      try {
        await sendMessageUser(driverData._id, messageText);
    ...
    const handleImageUpload = async () => {
      try {
        ...
        if (!result.canceled) {
          ...
          // You can handle the selected image URI here, e.g., upload it to a server
        } else {
