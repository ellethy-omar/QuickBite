/**
 * @swagger
 * /api/user/getActiveChats:
*   get:
*     tags:
*       - User (JWT required)
*     summary: Retrieve active chat previews for the authenticated user
*     description: |
*       Returns all active chats in which the user participates.
*       Each entry includes:
*         - chat ID
*         - the user participant’s basic info (name, phone, profilePicture)
*         - the associated order’s ID, status, and totalAmount
*         - the most recent message preview (content, timestamp, whether it was sent by user)
*         - the count of unread messages directed to the user
*     responses:
*       200:
...
*     tags:
*       - User (JWT required)
*     summary: Retrieve and mark as read this user’s chat messages
*     description: |
...
*       All returned messages will be marked as read for the user.
*     parameters:
*       - name: chatId
*         in: query
